...
export const searchSeed: SearchEntry[] = [
  ...
  ...courses.map((course) => ({ label: course.title, sublabel: course.provider, route: '/learning' })),
  ...officers.map((officer) => ({ label: officer.name, sublabel: `${officer.designation} · ${officer.department}`, route: '/directory' })),
]